...
- **多时间框架 ATR**: 网格间距采用 30m/1h/2h/4h/6h/8h/12h/1d 多时间框架 ATR 动态计算
- **Fibonacci 加仓**: 安全订单数量按 Fibonacci 序列（1,2,3,5,8,13,21,34,55）递增
...
3. **网格下单 (placeGridOrders)**: 根据多时间框架 ATR 计算间距，Fibonacci 序列计算数量，放置限价安全单
4. **止盈管理 (updateTP)**: 基于 30 分钟 ATR 计算止盈价格，均价 + ATR 放置限价卖出单
5. **加仓处理**: 安全单成交后，重新计算均价与止盈价格，更新 TP 订单
6. **止盈退出**: TP 订单成交后，取消所有挂单，状态重置为 IDLE，准备下一轮

### 网格间距设计

网格采用**多时间框架 ATR 分段**设计，不同层级使用不同时间框架的 ATR 值：

| 层级 | 间距计算 | 时间框架 | 说明 |
|------|----------|----------|------|
| 1 | ATR(30m) | 30 分钟 | 首层保护，间距较小 |
| 2 | ATR(1h) | 1 小时 | 第二层保护 |
| 3 | ATR(30m) + ATR(1h) | 组合 | 复合间距，加大缓冲 |
| 4 | ATR(30m) + ATR(2h) | 组合 | 复合间距，加大缓冲 |
| 5 | ATR(4h) | 4 小时 | 中長期保护 |
| 6 | ATR(6h) | 6 小时 | 中长期保护 |
| 7 | ATR(8h) | 8 小时 | 长期保护 |
| 8 | ATR(12h) | 12 小时 | 长期保护 |
| 9 | ATR(1d) | 1 天 | 最深层保护 |
